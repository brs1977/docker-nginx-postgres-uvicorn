...
def test_create_user(test_app, monkeypatch):
    ...
    response = test_app.post("/api/v1/users/", content=json.dumps(test_request_payload),)
...
def test_create_user_invalid_json(test_app):
    response = test_app.post("/api/v1/users/", content=json.dumps({"names": "Пользователь 4"}))
...
def test_read_user_incorrect_id(test_app, monkeypatch):
    ...
    response = test_app.get("/api/v1/users/999")
    assert response.status_code == 404
    assert response.json()["detail"] == "User not found"

    response = test_app.get("/api/v1/users/0")
    assert response.status_code == 422
...
def test_update_user(test_app, monkeypatch):
    ...
    response = test_app.put("/api/v1/users/1/", content=json.dumps(test_update_data))
    assert response.status_code == 200
...
def test_update_user_invalid(test_app, monkeypatch, id, payload, status_code):
    ...
    response = test_app.put(f"/api/v1/users/{id}/", content=json.dumps(payload),)
...
def test_remove_user(test_app, monkeypatch):
    ...
    response = test_app.delete("/api/v1/users/1/")
    assert response.status_code == 200
    assert response.json() == test_data
...
def test_remove_user_incorrect_id(test_app, monkeypatch):
    ...
    response = test_app.delete("/api/v1/users/999/")
    assert response.status_code == 404
    assert response.json()["detail"] == "User not found"

    response = test_app.delete("/api/v1/users/0/")
    assert response.status_code == 422